...
module.exports = async (req,res,next)=>{
    ...
    if(!user){
        ...
        try{
            ...
            logger.info(`updating single orders of user ${user.email}`);
            await bricklink.order.single(user,req.params.order_id).then(async(s)=>{
                if(s===false){
                    logger.warn(`ordersSingle was not successful for user ${user.email}, retrying in 20sec...`);
                    s = timeout(await bricklink.order.single,TIMEOUT_RESTART,user);
                    if(s===false){
                        res.send({success:false});
                        return;
                    }else{
